...
class UserController extends Controller
{
    ...
    public function organizerStatus($id)
    {
        $item = User::findOrFail($id);
        
        $item->admin_verified = $item->admin_verified == '1' ? '0' : '1';
        $item->save();
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->all();
        $organization_name = User::where('id',$request->organization)->first()->organization_name;
    ...
    public function update(Request $request, User $user)
    {
        ...
        if (array_intersect($roleNames, ['admin', 'scanner'])) {
            OrganizerPaymentKeys::where('organizer_id', $user->id)->delete();
        } else {
    ...
    {
        $data = Order::with(['customer:id,first_name,last_name,email', 'event:id,name'])->where('payment_status', 1);
