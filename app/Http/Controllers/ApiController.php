...
class ApiController extends Controller
{
    ...
    public function getEventsByLabel($label = null)
    {

        
        if (!$label) {
            return response()->json(['message' => 'Label is required.'], 400);
        }
    
        \Log::info('Label received: ' . $label);

        $label_data = label::where('name',$label)->first();
        
        if(empty($label_data))
        {
            return response()->json(['message' => 'No label found .'], 404);
        }
        else

        {
            $events = Event::where('label_id', $label_data['id'])->where('status',1)->where('is_deleted', 0)->where('event_status', 'Pending')->get();
            \Log::info('Events fetched: ', $events->toArray());
        
            if ($events->isEmpty()) {
                return response()->json(['message' => 'No events found for the specified label.'], 404);
            }
