...
class LicenseController extends Controller
{
    ...
    public function adminLogin(Request $request)
    {
        // Redirect to login page if request is GET
        if ($request->isMethod('get')) {
        ...
        if (Auth::attempt($credentials, $remember)) {
            ...
            if ($user->admin_verified != 1) {
                Auth::logout();
                return redirect('user/login')->with('error_msg', 'Account is not verified by the admin. Please wait!');
            }
            ...
            if ($user->hasRole('Organizer')) {
                ...
                Auth::logout();
                return redirect('user/login')->with('error_msg', 'Only authorized persons can log in.');
            }
        ...
        return redirect('user/login')->with('error_msg', 'Invalid Username or Password.');
    }
