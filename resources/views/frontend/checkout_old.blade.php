...
@section('content')
    ...
    <div class="pb-20 bg-scroll min-h-screen" style="background-image: url('images/events.png')">
        ...
        {{--
        <input type="hidden" name="email" value="manish86510@gmail.com">
        <input type="hidden" name="phone" value="8651051260">
        <input type="hidden" name="name" value="Manish Kumar">
         --}}
        <div id="ticketorder">
            @csrf
                        ...
                        <div
                            ...
                            <div class="">
                                <div
                                    ...
                                    <!--    class="rounded-lg w-56 h-56">-->
                                        @php
                                            ...
                                            if (is_array($data->event->image)) {
                                                $firstMedia = $data->event->image[0] ?? null;
                                            } elseif (is_string($data->event->image)) {
                                                if (Str::startsWith($data->event->image, '[')) {
                                                    $decoded = json_decode($data->event->image, true);
                                                    $firstMedia = is_array($decoded) ? $decoded[0] ?? null : null;
                                                } else {
                                                    $firstMedia = $data->event->image;
                                                }
                                            }
                                        ...
                                        @if ($mediaPath)
                                            @if ($isImage)
                                                <img src="{{ asset($mediaPath) }}" alt="{{ $data->event->name ?? '' }}"
                                    ...
                                    <div class="ml-4 2xl:w-[60%] xl:w-full xlg:w-full xmd:w-full xxmd:w-[80%]">
                                        ...
                                        <p class="font-poppins font-medium text-base leading-7 text-gray text-left">
                                            ...
                                            @if (Carbon\Carbon::parse($data->event->start_time)->format('d M Y') === Carbon\Carbon::parse($data->event->end_time)->format('d M Y'))
                                ...
                                <div
                                    ...
                                    <div class="ml-8 2xl:w-[60%] xl:w-full xlg:w-full xmd:w-full xxmd:w-[80%]">
                                      <!-- Buttons remain same -->
                                      ...
                                      <div id="termsModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
                                        <div class="bg-white rounded-2xl shadow-2xl w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] xl:w-[50%] h-[80vh] flex flex-col">
                                                        ...
                                                        <script>
                                                            ...
                                                            function toggleRazorRadio() {
                                                                if (nameInput.value.trim() !== '' &&
                                                                    emailInput.value.trim() !== '' &&
                                                                    phoneInput.value.trim() !== '') {
                                                                    razorRadio.disabled = false;
                                                                } else {
                                                                    razorRadio.disabled = true;
                                                                }
                                                            }
                                                ...
                                                @if ($setting->razor == 1)
                                                    ...
                                                    @else
                                                       <div class="border border-gray-light p-5 rounded-lg text-gray-100 w-full font-normal font-poppins text-base leading-6">
                                                        ...
                                                        <!-- GST Modal -->
                                                        <div id="gstModal" class="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50 hidden">
                                                            <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/3 p-6 relative">
                                                                <h2 class="text-xl font-semibold mb-4 text-gray-800">Enter GST Number</h2>
                                                                <input type="text" id="gst_number_input" placeholder="Enter GST Number"
                                                                    class="w-full border border-gray-300 rounded p-2 text-gray-800 font-poppins">
                                                                <div class="flex justify-end mt-4">
                                                                    <button type="button" id="cancelGST"
                                                                        class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 mr-2">Cancel</button>
                                                                    <button type="button" id="saveGST"
                                                                        class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 mr-2">Save</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <script>
                                                            ...
                                                            // Handle GST checkbox click (opens modal on first click)
                                                            claimGSTCheckbox.addEventListener('change', function(e) {
                                                                if (this.checked && !gstNumberHidden.value) {
                                                                    // Only open modal if GST number isn't already saved
                                                                    e.preventDefault(); // Prevent immediate checkbox check
                                                                    gstModal.classList.remove('hidden');
                                                                } else if (!this.checked) {
                                                                    // Clear GST number if unchecked
                                                            ...
                                                            saveGSTBtn.addEventListener('click', function() {
                                                                ...
                                                                if (gstNumber) {
                                                                    gstNumberHidden.value = gstNumber; // Store in hidden field
                                                                    claimGSTCheckbox.checked = true; // Check the checkbox
                                                                    gstModal.classList.add('hidden'); // Close modal
                                                                    gstNumberInput.value = ''; // Clear input for next time
                                                                } else {
                                                            ...
                                                            cancelGSTBtn.addEventListener('click', function() {
                                                                claimGSTCheckbox.checked = false; // Keep checkbox unchecked
                                                                gstModal.classList.add('hidden'); // Close modal
                                                                gstNumberInput.value = ''; // Clear input
                                                            });
                                                        </script>
                                                    @endif
                        ...
                        <div class="xlg:w-[25%] xxmd:w-full xxsm:w-full">
                            <div class="p-4 bg-white shadow-lg rounded-md space-y-5">
                                <p class="font-poppins font-semibold text-2xl leading-8 text-black pb-3">
                                    {{ __('Payment Summary') }}</p>
                                <div
                                    class="flex justify-between border border-primary rounded-md py-5 xxsm:flex-wrap sm:flex-nowrap xlg:px-0" style="justify-content: space-evenly;align-items: center;">
                                    <input type="text" value="" name="coupon_code" id="coupon_id"
                                        class="focus:outline-none font-poppins font-normal text-base leading-6 text-white-100 ml-5 1xl:w-44 xl:w-36
                            xlg:w-28"
                                        placeholder="{{ __('Coupon Code') }}">
