        ...
        <div class="container mx-auto px-4">
            <div class="flex items-center justify-between py-4">
                <div>
                    ...
                    <a href="{{ url('/') }}" class="object-cover ">
                        <img src="{{ $logo ? url('images/upload/' . $logo) : asset('/images/logo.png') }}"
                            class="object-scale-down" alt="Logo" style="height:100px">
                    </a>
                </div>
                <div class="sm:flex sm:items-center">
                    <ul class="navbar-nav flex space-x-4 xxsm:max-xl:hidden">
                        ...
                        <!--<li class="xxsm:max-lg:hidden nav-item {{ Request::is('partners') ? 'active' : '' }}">-->
                        <!--    <a href="{{ url('/partners') }}"-->
                        <!--        class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">{{ __('Partners') }}</a>-->
                        <!--</li>-->
                                            ...
                                            @if (!empty($categories) && is_iterable($categories))
                                                @foreach ($categories as $item)
                                                    <div class="flex items-left justify-left">
                                                        <a href="{{ url('/events-category/' . ($item->id ?? '0') . '/' . ($item->name ?? 'Unknown')) }}"
                                                            class="flex items-left text-base font-poppins font-normal leading-5 {{ isset($catactive) && $catactive == ($item->name ?? '') ? ' text-primary capitalize p-2 bg-primary-light' : '' }} capitalize p-2 hover:bg-primary-light rounded-md w-full">
                        ...
                        <li class="xxsm:max-lg:hidden nav-item {{ Request::is('how-we-help') ? 'active' : '' }}">
                            <a href="{{ url('/how-we-help') }}"
                                class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">{{ __('How We Help') }}</a>
                        </li>
                ...
                <div class="sm:flex sm:gap-3 sm:items-center">
                    ...
                    @if (Auth::guard('appuser')->check() || Auth::check())
                        @php
                            $currentUser = Auth::guard('appuser')->check() ? Auth::guard('appuser')->user() : Auth::user();
                            $logoutUrl = Auth::guard('appuser')->check() ? '/user/logoutuser' : '/logout';
                        @endphp
                        <div class="flex justify-center relative dropdownScreenButton cursor-pointer">
                                ...
                                <p class="font-poppins font-medium text-sm leading-5 text-black">
                                    @if(Auth::check())
                                        <a href="{{ url('/organization/home') }}"
                                            class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">
                            ...
                            <div>
                                <img src="{{ asset('images/upload/' . $currentUser->image) }}"
                                    class="w-10 h-10 bg-cover object-contain border border-gray-light rounded-full"
                                    alt="Profile Image">
                            </div>
                            <div class="ml-3 pt-1 dropdown relative flex">
                                <div class="relative inline-block text-left">
                                    <div class="dropdownMenuClass hidden origin-top-right absolute right-0 w-56 rounded-md shadow-2xl z-30 mt-10">
                                            ...
                                            <div class="py-1">
                                                    ...
                                                    <ul class="space-y-8">
                                                        <li>
                                                            <a href="{{ url('/my-tickets') }}"
                                                                class="flex items-center font-normal font-poppins leading-6 text-black text-base capitalize">
                                                                {{ __('My tickets') }}
                                                            </a>
                                                        </li>
                                                        <li>
                                                ...
                                                <div class="px-3 py-5">
                                                    <button type="button" id="logout-button"
                                                        ...
                                                        <span class="flex-1 whitespace-nowrap">{{ __('Logout') }}</span>
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            document.getElementById("logout-button").addEventListener('click', function() {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        document.location = "{{ url($logoutUrl) }}";
                                    }
                                });
                            });
                        </script>
                    @else
                        <div>
                            <button onclick="window.location.href='{{ url('user/login') }}'" class="create-event-btn">
                                <img src="{{ asset('images/ticket-icon.svg') }}" class="w-5 h-5" alt="">
                                CREATE EVENT
                            </button>
                        </div>
                        <div class="sm:px-4 flex xxsm:flex-wrap justify-end mt-2 sm:mt-0">
                            <a href="{{ url('user/login') }}"
                                class="px-5 py-2 text-white bg-primary text-center font-poppins font-normal text-base leading-6 rounded-md">
                                Sign In
                            </a>
                        </div>
                    @endif
            ...
            <div class="hidden nav-content" id="nav-content">
                <ul class="list-reset lg:flex gap-5 justify-end flex-1 items-center text-end pb-5">
                    ...
                    <!--<li class="mt-2 nav-item {{ Request::is('about') ? 'active' : '' }}">-->
                    <!--    <a href="{{ url('/about') }}"-->
                    <!--        class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">{{ __('About Us') }}</a>-->
                    <!--</li>-->
                    <li class="mt-2 nav-item {{ Request::is('all-events') ? 'active' : '' }}">
                        <a href="{{ url('/all-events') }}"
                            class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">{{ __('Events') }}</a>
                    </li>
                    <!--<li class="mt-2 nav-item {{ Request::is('partners') ? 'active' : '' }}">-->
                    <!--    <a href="{{ url('/partners') }}"-->
                    <!--        class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">{{ __('Partners') }}</a>-->
                    <!--</li>-->
                    ...
                    <li class="mt-2 nav-item {{ $activePage == 'how-we-help' ? 'active' : '' }}">
                        <a href="{{ url('/how-we-help') }}"
                            class="nav-link md:px-1 capitalize font-poppins font-normal text-base leading-6 text-gray">{{ __('How We Help') }}</a>
                    </li>
     ...
     3xl:mx-52 2xl:mx-28 1xl:mx-28 xl:mx-10 xlg:mx-10 lg:mx-35 xxmd:mx-24 xmd:mx-32 sm:mx-20 msm:mx-16 xsm:mx-5 xxsm:mx-5 py-3 pt-4 z-30 xl2:flex-row">
        <div class="flex items-center lg:items-left w-auto">
            <ul
                ...
                <a href="{{ url('/') }}" class="object-cover ">
                    <img src="{{ $logo ? url('images/upload/' . $logo) : asset('/images/logo.png') }}"
                        class="object-scale-down h-10 " alt="Logo" style="height:50px">
                </a>
            </ul>
            ...
            <div class="flex justify-between sm:space-x-6 xxsm:flex-col sm:flex-row">
                ...
                @if (Auth::guard('appuser')->check() || Auth::check())
                    ...
                    <div class="flex justify-end mt-2 dropdownScreenButton">
                        ...
                        <div>
                            <img src="{{ asset('images/upload/' . $currentUser->image) }}" class="w-10 h-10 bg-cover object-contain border border-gray-light rounded-full" alt="Profile Image">
                        </div>
                        <div class="ml-3 pt-1 dropdown relative flex">
                            <div class="relative inline-block text-left">
                                <div class="dropdownMenuClass hidden origin-top-right absolute right-0 w-56 rounded-md shadow-2xl z-30 mt-10">
                                    <div class="rounded-md bg-white shadow-xs">
                                        ...
                                        <div class="py-1">
                                            <div class="overflow-y-auto py-4 px-3 bg-gray-50 rounded pt-10 border-b border-gray-light pb-5">
                                                <ul class="space-y-8">
                                                    <li>
                                                        <a href="{{ url('/my-tickets') }}" class="flex items-center font-normal font-poppins leading-6 text-black text-base capitalize">{{ __('My Tickets') }}</a>
                                                    </li>
                                                    <li>
                                                        <a href="{{ url('/user/profile') }}" class="flex items-center font-normal font-poppins leading-6 text-black text-base capitalize">{{ __('Profile') }}</a>
                                                    </li>
                                                    <li>
                                                        <a href="{{ url('/change-password') }}" class="flex items-center font-normal font-poppins leading-6 text-black text-base capitalize">{{ __('Change Password') }}</a>
                                                    </li>
                                                    @if (!empty($wallet))
                                                        <li>
                                                            <a href="{{ route('myWallet') }}" class="flex items-center font-normal font-poppins leading-6 text-black text-base capitalize">{{ __('My Wallet') }}</a>
                                                        </li>
                                                    @endif
                                                </ul>
                                            </div>
                                        ...
                                        @endif
                                        <div class="px-3 py-5">
                                            <button type="button" id="logout-button-mobile" class="flex items-center font-poppins font-medium leading-4 text-danger capitalize">
                                                <i class="fa-solid fa-right-from-bracket mr-2"></i>
                                                <span class="flex-1 whitespace-nowrap">{{ __('Logout') }}</span>
                                            </button>
                                        </div>
                ...
                @else
                    ...
                    <div class="sm:px-4 flex xxsm:flex-wrap justify-end mt-2 sm:mt-0 gap-5">
                        <div class="sm:px-4 flex xxsm:flex-wrap justify-end mt-2 sm:mt-0">
                            <button onclick="window.location.href='{{ url('user/login') }}'" class="create-event-btn">
                                <img src="{{ asset('images/ticket-icon.svg') }}" class="w-5 h-5" alt="">
                                CREATE EVENT
                        ...
                        <div class="sm:px-4 flex xxsm:flex-wrap justify-end mt-2 sm:mt-0">
                            <a href="{{ url('user/login') }}" class="px-5 py-2 text-white bg-primary text-center font-poppins font-normal text-base leading-6 rounded-md">
                                Sign In
                            </a>
                        </div>
                    </div>
